...
models <- prob %>%
  ...
  mutate(
    ...
    same_slope = map(data,
                     ~glm(cbind(k, r) ~ size + log10_duration - 1,
                          data = ., family = binomial(mafc.logit(2)))),
    aic_same_model = map_dbl(same_slope, "aic"),
    dev_same_model = map_dbl(same_slope, "deviance")
  )
...
### Comparing same and different slope
model_comparisons %>% filter(p.value < alpha)
model_comparisons %>% filter(p.value < alpha) %>% pull(anov2)
...
model_dif_slope <- models %>% dplyr::select(-same_slope)
...
my_logistic_fun <- function(x, p) .5 + .5 * (1 + exp( -p[2] * (x - p[1])) )^(-1)
my_logistic_fun2 <- function(x, p) .5 + .5 * (1 + exp(-p[2] * (x - p[3])))^(-1)

model_same_slope_quickpsy <-
  quickpsy(dat_resp %>%
            mutate(log10_duration = log10(duration)),
           log10_duration, correct,
           fun = tibble(size = c("Large", "Small"),
                        fun = c(my_logistic_fun, my_logistic_fun2)),
           parini = c(-1, 10, -1),
           grouping = .(platform, participant, size),
           bootstrap = "none",
           xmin = -2.3, xmax = -.5)

### Comparing same and different slope using quickpsy
loglik_dif <- model_dif_slope_quickpsy$logliks %>%
  group_by(platform, participant) %>%
  summarise(loglik_dif = sum(loglik))

loglik_same <- model_same_slope_quickpsy$logliks %>%
  select(-n_par) %>%
  rename(loglik_same = loglik)

loglik_dif %>%
  left_join(loglik_same) %>%
  mutate(deviance =  2 * (loglik_dif - loglik_same),
         p.value =  pchisq(deviance, 1, lower.tail = FALSE)) %>%
...
psychometric_functions_dif <- model_dif_slope %>%
  calculate_predictions_dif(log10_duration_seq)
...
p_size_dif <- ggplot(prob) +
  facet_grid(platform~ participant, scales = "free") +
  geom_line(data = psychometric_functions_dif, size = .5 * size_line,
            aes(x = duration,y = .fitted,  color = size, lty = "Dif slopes glm")) +
  geom_line(data = model_dif_slope_quickpsy$curves, size = .5 * size_line,
            aes(x = 10^x, y = y,  group = size, lty = "Dif slopes quickpsy")) +
...
p_size_same <- ggplot(prob) +
  facet_grid(platform~ participant, scales = "free") +
  geom_line(data = psychometric_functions, size = .5 * size_line,
            aes(x = duration,y = .fitted,  color = size, lty = "Same slope glm")) +
  geom_line(data = model_same_slope_quickpsy$curves, size = .5 * size_line,
            aes(x = 10^x, y = y,  group = size, lty = "Same slope quickpsy")) +
  geom_point(aes(x = duration, y = prob,
                 color = size, shape = size), size = size_point) +
  geom_segment(data = conf_int_size,
               aes(x = threshold_min, xend = threshold_max,
...
p_psycho_comparison <- plot_grid(p_size_dif, p_size_same, ncol = 1,
                                 labels = "AUTO")

ggsave("figures/p_psycho_comparison.pdf", p_psycho_comparison,
...
### Goodness of fit using quickpsy and the chi sqr distribution
model_same_slope_quickpsy$deviance %>%
